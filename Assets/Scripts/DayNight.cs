...
public class DayNight : MonoBehaviour {
    ...
    void Start() {
        ...
        moon.name = "moon";
        renderer = moon.GetComponent<Renderer>();
    ...
    public Vector2 rotationOffset = new Vector2(70, 70);
    ...
    void Update() {
        ...
        midpoint.y -= 0.5f; //midpoint = playerposition at floor height
        sun.transform.position = midpoint + Quaternion.Euler(rotationOffset.x, rotationOffset.y, sunangle) * (radius * Vector3.right);
        sun.transform.LookAt(midpoint);
        moon.transform.position = midpoint + Quaternion.Euler(rotationOffset.x, rotationOffset.y, moonangle) * (radius * Vector3.right);
